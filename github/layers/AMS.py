...
class AMS(nn.Module):
    def __init__(self, input_size, output_size, num_experts, device, num_nodes=7, d_model=32, d_ff=64, dynamic=False,
    ...
    def seasonality_and_trend_decompose(self, x):
        ...
        # 打印 seasonality 的形状和前 5 行数值
        # print(f"seasonality 的形状: {seasonality.shape}")
        # print("seasonality 的前 5 行数值:")
        # print(seasonality[:5])

        # 打印 trend 的形状和前 5 行数值
        # print(f"trend 的形状: {trend.shape}")
        # print("trend 的前 5 行数值:")
        # print(trend[:5])
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        # 打印 expert_inputs 的相关信息
        #  print(f"expert_inputs 的长度: {len(expert_inputs)}")
        #  for i, input_i in enumerate(expert_inputs):
            #  print(f"expert_inputs 中第 {i} 个元素的形状: {input_i.shape}")
        #   # 打印前几个元素的值
        #   try:
        #    print(f"expert_inputs 中第 {i} 个元素的前几个值: {input_i[:3].detach().cpu().numpy()}")   0=([30, 96, 7, 16])，1=([33, 96, 7, 16])，2=([39, 96, 7, 16])， 3=([26, 96, 7, 16])
        #                                                                                           0对应第0个专家 patch=8 （分别对应0-3专家的patch[8, 6, 4, 2]）
        #   except IndexError:
        #   print(f"expert_inputs 中第 {i} 个元素长度不足 3，全部值为: {input_i.detach().cpu().numpy()}")


        # expert_outputs = [self.experts[i](expert_inputs[i])[0] for i in range(self.num_experts)]
        expert_outputs = []
        for i in range(self.num_experts):
            output = self.experts[i](expert_inputs[i])[0]   # 将 expert_inputs[i]（即第 i 个专家的输入数据）传递给 self.experts[i]（第 i 个专家模型）进行计算
